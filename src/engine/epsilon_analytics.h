/* epsilon_analytics.h - per-node workload and performance analytics.
 *
 ...
 * produce the full cluster picture via edb_analytics_report().
 *
 * Snapshots are rewritten every EDB_ANALYTICS_FLUSH_SECS and carry a
 * EDB_ANALYTICS_TTL_SECS TTL so a node that goes away stops contributing
 * after 30 minutes without leaving unbounded data behind.
 */

#ifndef EPSILON_ANALYTICS_H
#define EPSILON_ANALYTICS_H
...
#define EDB_ANALYTICS_KEYSPACE    "config_analytics"
#define EDB_ANALYTICS_FLUSH_SECS  10
#define EDB_ANALYTICS_TTL_SECS    1800

typedef struct edb_analytics edb_analytics;
...
typedef bool (*edb_analytics_flush_fn)(void *ctx, const char *node_id,
...
typedef cJSON *(*edb_analytics_cluster_fn)(void *ctx);
 ...
 * edb_analytics_cluster_fn). Call before/after start; safe on NULL. */
