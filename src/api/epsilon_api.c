...
{
    if (strcmp(db, EDB_SYSTEM_DB) == 0) {
        /* config writes never run scripts */
        if (reason) {
            *reason = NULL;
        }
        return EDB_LUA_OK;
    }
    edb_lua_ctx ctx = { g_ctx.engine, g_ctx.config, g_repl, g_cluster };
    edb_lua_event_arg arg = { db, part, ks, id, value, groups, trusted };
...
static bool api_apply_change_impl(void *ud, const cJSON *change)
{
    ...
    /* capture the pre-change document so after_* handlers see
     * the written (or deleted) record and inserts/updates can be told
     * apart; borrows for the event, freed afterwards */
    cJSON *deleted = NULL;
    bool existed = false;
    bool is_put = strcmp(operation->valuestring, "put") == 0;
    bool ok = false;
    if (is_put) {
        ...
        cJSON *prior = edb_get(g_ctx.engine, partition->valuestring,
                               keyspace->valuestring, id->valuestring);
        existed = prior != NULL;
        cJSON_Delete(prior);
        ...
        deleted = edb_get(g_ctx.engine, partition->valuestring,
                          keyspace->valuestring, id->valuestring);
    ...
    if (ok) {
        /* replicas run after_* scripts on replay, unless the
         * change was itself generated by a script ("lua":true) */
        const cJSON *lua_mark = cJSON_GetObjectItemCaseSensitive(change,
                                                                 "lua");
        if (!cJSON_IsBool(lua_mark)) {
            char *veto = NULL;
            if (is_put) {
                const cJSON *value =
                ...
                if (cJSON_IsObject(value)) {
                    written = cJSON_Duplicate(value, 0);
                }
                (void)api_fire_lua(
                    existed ? EDB_LUA_AFTER_UPDATE : EDB_LUA_AFTER_INSERT,
                    database->valuestring, partition->valuestring,
                    keyspace->valuestring, id->valuestring, &written, 0,
                    true, &veto);
                cJSON_Delete(written);
            } else {
                (void)api_fire_lua(
                    EDB_LUA_AFTER_DELETE, database->valuestring,
                    partition->valuestring, keyspace->valuestring,
                    id->valuestring, &deleted, 0, true, &veto);
            }
            free(veto);
        }
    }
                      ...
                      edb_config *config)
{
    ...
    /* EQL: POST /eql on the client port; POST /admin/eql works
    ...
    ok &= edb_http_add_handler(srv, "POST", "/admin/eql",
                               handle_admin_eql);
    /* Lua scripting admin surface */
    ok &= edb_http_add_handler(srv, "GET", "/admin/code",
                               handle_admin_code);
    ok &= edb_http_add_handler(srv, "POST", "/admin/code",
                               handle_admin_code);
    ok &= edb_http_add_handler(srv, "POST", "/admin/code/validate",
                               handle_admin_code_validate);
    ok &= edb_http_add_handler(srv, "GET", "/admin/code/",
                               handle_admin_code_item);
    ok &= edb_http_add_handler(srv, "DELETE", "/admin/code/",
                               handle_admin_code_item);
