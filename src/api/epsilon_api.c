...
static bool api_apply_change_impl(void *ud, const cJSON *change)
{
    ...
    /* stage 9: capture the pre-change document so after_* handlers see
    ...
    if (ok) {
        /* stage 9: replicas run after_* scripts on replay, unless the
                      ...
                      edb_config *config)
{
    ...
    ok &= edb_http_add_handler(srv, "POST", "/admin/eql",
                               handle_admin_eql);
    /* stage 9: Lua scripting admin surface */
