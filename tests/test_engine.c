/* Plain C test harness for the EpsilonDB shard engine. */
...
#include "../src/engine/epsilon_engine.h"
...
static void test_put_get_delete(void)
{
    ...
    edb_engine *mgr = edb_engine_open("tests/data/basic");
    CHECK(mgr != NULL);

    const char *doc = "{\"name\":\"alice\",\"age\":30}";
    CHECK(edb_put(mgr, "users", "profiles", "u1", doc, -1));

    cJSON *got = edb_get(mgr, "users", "profiles", "u1");
    CHECK(got != NULL);
    ...
    /* overwrite */
    CHECK(edb_put(mgr, "users", "profiles", "u1",
                  "{\"name\":\"alice2\",\"age\":31}", -1));
    got = edb_get(mgr, "users", "profiles", "u1");
    CHECK(got != NULL && json_string_eq(got, "name", "alice2"));
    cJSON_Delete(got);

    /* missing key */
    CHECK(edb_get(mgr, "users", "profiles", "nope") == NULL);

    /* soft delete */
    CHECK(edb_delete(mgr, "users", "profiles", "u1"));
    CHECK(edb_get(mgr, "users", "profiles", "u1") == NULL);
...
static void test_ttl_expiry(void)
{
    ...
    edb_engine *mgr = edb_engine_open("tests/data/ttl");
    CHECK(mgr != NULL);

    CHECK(edb_put(mgr, "sess", "tokens", "s1", "{\"v\":1}", 1));
    CHECK(edb_put(mgr, "sess", "tokens", "s2", "{\"v\":2}", -1));

    cJSON *got = edb_get(mgr, "sess", "tokens", "s1");
    CHECK(got != NULL);
    cJSON_Delete(got);

    sleep(2);

    CHECK(edb_get(mgr, "sess", "tokens", "s1") == NULL);
    got = edb_get(mgr, "sess", "tokens", "s2");
    CHECK(got != NULL);
    cJSON_Delete(got);
...
static void test_filters_and_query(void)
{
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        CHECK(edb_filters_valid(filters));
        cJSON *rows = edb_query(mgr, "kv", "main", filters);
    ...
    for (size_t i = 0; i < sizeof(invalid) / sizeof(invalid[0]); i++) {
        ...
        CHECK(!edb_filters_valid(filters));
        CHECK(edb_query(mgr, "kv", "main", filters) == NULL);
        cJSON_Delete(filters);
    }
    ...
    cJSON *nested = cJSON_Parse(
        "{\"key\":\"manager.age\",\"operator\":\"eq\",\"value\":42}");
    size_t count = 0;
    char **ids = edb_ids(mgr, "kv", "main", nested, &count);
    CHECK(ids && count == 1 && strcmp(ids[0], "b") == 0);
    edb_free_strings(ids);
    cJSON *all = edb_all(mgr, "kv", "main", nested);
    CHECK(all && cJSON_GetArraySize(all) == 1);
    cJSON_Delete(all);
    cJSON_Delete(nested);

    CHECK(edb_delete(mgr, "kv", "main", "b"));
    cJSON *age = cJSON_Parse(
        "{\"key\":\"age\",\"operator\":\"gte\",\"value\":42}");
    ids = edb_ids(mgr, "kv", "main", age, &count);
    ...
    char shard_key[33];
    CHECK(edb_shard_path(mgr, "kv", "main", shard_path, sizeof(shard_path),
                         shard_key));
    sqlite3 *db = NULL;
    ...
    edb_engine_close(mgr);
    mgr = edb_engine_open("tests/data/filters");
    CHECK(mgr != NULL);
    cJSON *persisted = cJSON_Parse(
        "{\"key\":\"manager.age\",\"operator\":\"lt\",\"value\":40}");
    cJSON *rows = edb_query(mgr, "kv", "main", persisted);
...
static void test_shard_layout_and_reopen(void)
{
    ...
    edb_engine *mgr = edb_engine_open("tests/data/layout");
    CHECK(mgr != NULL);

    CHECK(edb_put(mgr, "p1", "k1", "x", "{\"v\":1}", -1));
    CHECK(edb_put(mgr, "p1", "k2", "x", "{\"v\":2}", -1));
    CHECK(edb_put(mgr, "p2", "k1", "x", "{\"v\":3}", -1));
    CHECK(edb_put(mgr, "ab", "c", "same", "{\"v\":4}", -1));
    CHECK(edb_put(mgr, "a", "bc", "same", "{\"v\":5}", -1));

    CHECK(count_shards("tests/data/layout") == 5);
    cJSON *left = edb_get(mgr, "ab", "c", "same");
    cJSON *right = edb_get(mgr, "a", "bc", "same");
    ...
    mgr = edb_engine_open("tests/data/layout");
    CHECK(mgr != NULL);
    size_t n = 0;
    char **ids = edb_ids(mgr, "p2", "k1", NULL, &n);
    CHECK(ids != NULL && n == 1);
    edb_free_strings(ids);

    cJSON *got = edb_get(mgr, "p1", "k2", "x");
...
static void test_cleanup_pass(void)
{
    ...
    edb_engine *mgr = edb_engine_open("tests/data/cleanup");
    CHECK(mgr != NULL);

    CHECK(edb_put(mgr, "tmp", "main", "gone", "{\"v\":1}", 1));
    CHECK(edb_delete(mgr, "tmp", "main", "gone"));

    /* row is invisible but still present until cleanup + grace window */
    CHECK(edb_get(mgr, "tmp", "main", "gone") == NULL);

    /* force cleanup pass; grace is 2h so nothing should be purged yet */
    CHECK(edb_force_cleanup(mgr, "tmp", "main"));

    /* engine still functional after cleanup */
    CHECK(edb_put(mgr, "tmp", "main", "fresh", "{\"v\":9}", -1));
    cJSON *got = edb_get(mgr, "tmp", "main", "fresh");
    CHECK(got != NULL);
    cJSON_Delete(got);
...
static void test_concurrent_puts(void)
{
    ...
    edb_engine *mgr = edb_engine_open("tests/data/concurrent");
    CHECK(mgr != NULL);
    ...
    for (int w = 0; w < writers; w++) {
        ...
        if (pid == 0) {
            ...
            for (int i = 0; i < per_writer; i++) {
                ...
                if (!edb_put(mgr, "load", "test", id, doc, -1)) {
                    _exit(1);
                }
            }
            _exit(0);
    ...
    size_t n = 0;
    char **ids = edb_ids(mgr, "load", "test", NULL, &n);
